...
public class CaseloadContentAction extends DispatchAction {
	...
	public ActionForward noteSearch(ActionMapping actionMapping,
	    ...
	    if( siteName!=null) {
	    	Site site = siteDao.getByLocation(siteName);
	    	if(site!=null) {
	    		siteId = String.valueOf(site.getSiteId());
	    	}
	    }
	    
		String clSearchQuery = null;
		String[] clSearchParams = null;
		
		if (org.oscarehr.common.IsPropertiesOn.isMultisitesEnable() ) {
			if(siteId!=null && !siteId.equalsIgnoreCase("none") ) {
				clSearchQuery="search_notes_sitefilter";
				clSearchParams = new String[] { caseloadProv, caseloadProv, caseloadProv, caseloadProv, "%"+caseloadQuery+"%", siteId };				
			} else {
				clSearchQuery="search_notes";
				clSearchParams = new String[] { caseloadProv, caseloadProv, caseloadProv, caseloadProv, "%"+caseloadQuery+"%" };				
			}							
		} else {
			clSearchQuery = "search_notes";
			clSearchParams = new String[] { caseloadProv, caseloadProv, caseloadProv, caseloadProv, "%"+caseloadQuery+"%"};
		...
		if (org.oscarehr.common.IsPropertiesOn.isMultisitesEnable() ) {
			...
			if (!StringUtils.isNullOrEmpty(caseloadDx) && !StringUtils.isNullOrEmpty(caseloadRoster)) {
				...
				if(siteId!=null && !siteId.equalsIgnoreCase("none") ) {
					...
					clSearchParams[0] = siteId;
				} else {
					clSearchQuery="search_alldemo_noSiteFilter";
			...
			if(siteId==null || siteId.equalsIgnoreCase("none"))
			{
				//and if there is atleast one filter (dx, ro) added, then append nosite
				if(!clSearchQuery.equalsIgnoreCase("search_alldemo_noSiteFilter"))
				{
					clSearchQuery = clSearchQuery+"_nosite";
				}
			}
		} else {
			...
			if (!StringUtils.isNullOrEmpty(caseloadDx) && !StringUtils.isNullOrEmpty(caseloadRoster)) {
				...
				if(siteId!=null && !siteId.equalsIgnoreCase("none") ) {
					...
					clSearchParams[2] = caseloadProv;
					clSearchParams[3] = caseloadProv;	
					clSearchParams[4] = siteId;
				} else {
					clSearchQuery="search_provdemo_noSiteFilter";				
