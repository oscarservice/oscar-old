...
public class BillingClaimDAO extends AbstractDao<BillingClaimHeader1> {
    ...
    private String calcTotal(List<String>codes, Date serviceDate) {
        ...
        for( String code : codes ) {
            ...
            if( !"".equalsIgnoreCase(billingservice.getPercentage())) {
    ...
    @SuppressWarnings("unchecked")
    public List<BillingClaimHeader1> getInvoicesByApptNo(String apptNo) {
    	String sql = "select h1 from BillingClaimHeader1 h1 where " +
                " h1.appointment_no = :apptNo and h1.status != 'D'";
        Query q = entityManager.createQuery(sql);
        
        q.setParameter("apptNo", new Integer(apptNo));
        
        return q.getResultList();
    }
    
    @SuppressWarnings("unchecked")
    public List<BillingClaimHeader1> getInvoices(String demographic_no, Integer limit,Date strStart,Date strToday) {
    	String sql = "select h1 from BillingClaimHeader1 h1 where " +
                " h1.demographic_no = :demo and  h1.billing_date>= :strStart and h1.billing_date<= :strToday and h1.status != 'D' order by h1.billing_date desc";
        Query q = entityManager.createQuery(sql);
        
        q.setParameter("demo", new Integer(demographic_no));
        q.setParameter("strStart", strStart);
        q.setParameter("strToday", strToday);
        q.setMaxResults(limit);
    ...
    @SuppressWarnings("unchecked")
    public List<BillingClaimHeader1> getInvoices(String demographic_no, Integer limit, Integer iOffset, Date strStart,Date strToday) {
    	String sql = "";
    	if (strStart != null && strToday != null){
    		sql = "select h1 from BillingClaimHeader1 h1 where " +
                    " h1.demographic_no = :demo and  h1.billing_date>= :strStart and h1.billing_date<= :strToday and h1.status != 'D' order by h1.billing_date desc";
    	} else {
    		sql = "select h1 from BillingClaimHeader1 h1 where " +
                    " h1.demographic_no = :demo and h1.status != 'D' order by h1.billing_date desc";
