...
public class EyeFormDao extends AbstractDao<EyeForm> {
	...
	public EyeForm getByAppointmentNo(int appointmentNo) {
		Query query = entityManager.createQuery("select x from "+modelClass.getSimpleName()+" x where x.appointmentNo=?1 order by date desc");
...
//			eyeform = (EyeForm)query.getSingleResult();
			List<EyeForm> eyeforms = query.getResultList();
			if(eyeforms.size()>0) eyeform = eyeforms.get(0);
