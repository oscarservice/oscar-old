...
public class CaseManagementEntryAction extends BaseCaseManagementEntryAction {
		...
		if (fwd.getName().equals("windowClose")) {
			...
			if (macro.getBillingCodes() != null && macro.getBillingCodes().length() > 0) {
				...
				if(StringUtils.isBlank(macro.getBillingBillto())) {
					...
					mockReq.addParameter("xml_provider", providerBilling.getProviderNo() + "|" + providerBilling.getOhipNo());
				}								
	...
	public ActionForward exitMacro(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
