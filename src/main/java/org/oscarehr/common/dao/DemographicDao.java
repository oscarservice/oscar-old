...
public class DemographicDao extends HibernateDaoSupport {
    ...
    public static List<Integer> getDemographicIdsAdmittedIntoFacility(int facilityId) {
     ...
     public List<Demographic> searchDemographic(String searchStr, String status){
         String fieldname = "", regularexp = "like";
         ...
         if(status!=null)
         {
        	 if(status.equalsIgnoreCase("active"))
        		 patientStatus = "patient_status not in ('IN','DE','IC','ID','MO','FI')";
        	 else if(status.equalsIgnoreCase("inactive"))
        		 patientStatus = "patient_status in ('IN','DE','IC','ID','MO','FI')";
         }
         
         String hql = "";
         if(patientStatus!=null && patientStatus.length()>0)
        	 hql = "From Demographic d where "+patientStatus+" and " + fieldname + " " + regularexp + " ? ";
         else
        	 hql = "From Demographic d where " + fieldname + " " + regularexp + " ? ";
