...
public class CaseloadDao {
	...
	private static void initializeSearchQueries() {
		...
		if (org.oscarehr.common.IsPropertiesOn.isMultisitesEnable()) { 
			...
			caseloadSearchQueries.put("search_notes_siteFilter", "select zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select distinct Z.demographic_no, Z.last_name, Z.first_name FROM (select distinct demographic_no, first_name, last_name, year_of_birth, month_of_birth, date_of_birth, sex from demographic left join demographiccust using (demographic_no) where (provider_no='%s' or cust1='%s' or cust2='%s' or cust4='%s') and patient_status not in ('FI','MO','DE','IN')) as Z INNER JOIN casemgmt_note using (demographic_no) where note like '%s' and locked <> '1' ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			
			//if site is selected
			caseloadSearchQueries.put("search_alldemo_rodxfilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select distinct demographic_no, last_name, first_name from dxresearch left join demographic using (demographic_no) where dxresearch_code='%s' and status='A' and patient_status not in ('FI','MO','DE','IN') and roster_status='%s') as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			caseloadSearchQueries.put("search_alldemo_dxfilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select distinct demographic_no, last_name, first_name from dxresearch left join demographic using (demographic_no) where dxresearch_code='%s' and status='A' and patient_status not in ('FI','MO','DE','IN') ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			caseloadSearchQueries.put("search_alldemo_rofilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select demographic_no, last_name, first_name from demographic where patient_status not in ('FI','MO','DE','IN', '0') and roster_status='%s' ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			caseloadSearchQueries.put("search_alldemo_nofilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select demographic_no, last_name, first_name from demographic where patient_status not in ('FI','MO','DE','IN') ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			caseloadSearchQueries.put("search_alldemo_noSiteFilter", "select demographic_no, last_name, first_name from demographic where patient_status not in ('FI','MO','DE','IN', '0')");			
			caseloadSearchQueries.put("search_provdemo_rodxfilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select distinct demographic_no, last_name, first_name from dxresearch left join demographic using (demographic_no) left join demographiccust using (demographic_no)  where (provider_no='%s' or cust1='%s' or cust2='%s' or cust4='%s')  and patient_status not in ('FI','MO','DE','IN')  and roster_status='%s' and dxresearch_code='%s' and status='A' ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			
			//if site is not selected
			caseloadSearchQueries.put("search_alldemo_rodxfilter_nosite", "select distinct demographic_no, last_name, first_name from dxresearch left join demographic using (demographic_no) where dxresearch_code='%s' and status='A' and patient_status not in ('FI','MO','DE','IN') and roster_status='%s'");
			caseloadSearchQueries.put("search_alldemo_dxfilter_nosite", "select distinct demographic_no, last_name, first_name from dxresearch left join demographic using (demographic_no) where dxresearch_code='%s' and status='A' and patient_status not in ('FI','MO','DE','IN') ");
			caseloadSearchQueries.put("search_alldemo_rofilter_nosite", "select demographic_no, last_name, first_name from demographic where patient_status not in ('FI','MO','DE','IN', '0') and roster_status='%s' ");
			caseloadSearchQueries.put("search_alldemo_nofilter_nosite", "select demographic_no, last_name, first_name from demographic where patient_status not in ('FI','MO','DE','IN')");
			
			caseloadSearchQueries.put("search_provdemo_dxfilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select distinct demographic_no, last_name, first_name from dxresearch left join demographic using (demographic_no) left join demographiccust using (demographic_no)  where (provider_no='%s' or cust1='%s' or cust2='%s' or cust4='%s')  and patient_status not in ('FI','MO','DE','IN') and dxresearch_code='%s' and status='A' ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			caseloadSearchQueries.put("search_provdemo_rofilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select distinct demographic_no, last_name, first_name from demographic left join demographiccust using (demographic_no) where (provider_no='%s' or cust1='%s' or cust2='%s' or cust4='%s') and patient_status not in ('FI','MO','DE','IN') and roster_status='%s' ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			caseloadSearchQueries.put("search_provdemo_nofilter", "select distinct zz.demographic_no, zz.last_name, zz.first_name from demographicSite ds left join (select distinct demographic_no, last_name, first_name from demographic left join demographiccust using (demographic_no) where (provider_no='%s' or cust1='%s' or cust2='%s' or cust4='%s') and patient_status not in ('FI','MO','DE','IN') ) as zz on ds.demographicId=zz.demographic_no where ds.siteId='%s'");
			caseloadSearchQueries.put("search_provdemo_noSiteFilter", "select distinct demographic_no, last_name, first_name from demographic left join demographiccust using (demographic_no) where (provider_no='%s' or cust1='%s' or cust2='%s' or cust4='%s') and patient_status not in ('FI','MO','DE','IN') ");
	...
	public Integer getCaseloadDemographicSearchSize(String searchQuery, String[] searchParams) {
		...
		demoQuery = String.format(caseloadSearchQueries.get(searchQuery), (Object[])searchParams);
		query = String.format("SELECT count(1) AS count FROM (%s) AS X WHERE demographic_no is not null", demoQuery);
