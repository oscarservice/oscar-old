...
public class ConsultationRequestDao extends AbstractDao<ConsultationRequest> {
	...
	{
		Query query = entityManager.createQuery("select count(c) from ConsultationRequest c where c.referralDate < ?1 and c.status != 4");
		query.setParameter(1, referralDateCutoff);

		return(Integer.parseInt(((Long)query.getSingleResult()).toString()));
	}

	public int getCountReferralsAfterCutOffDateAndNotCompleted(Date referralDateCutoff,String sendto)
	{
		Query query = entityManager.createQuery("select count(c) from ConsultationRequest c where c.referralDate < ?1 and c.status != 4 and c.sendTo = ?2");
		query.setParameter(1, referralDateCutoff);
        ...
        public int getCountConRequestWithSpecId(Integer specId)
    	{
    		Query query = entityManager.createNativeQuery("select * from consultationRequests where specId = ?1", modelClass);
    		query.setParameter(1, specId);

    		List<ConsultationRequest> list = (List<ConsultationRequest>)query.getResultList();
