...
public class UserPropertyDAO extends AbstractDao<UserProperty> {
    ...
    public UserProperty getProp(String prov, String name) {
    	Query query = entityManager.createQuery("select p from UserProperty p where p.providerNo = ? and p.name = ? order by id desc limit 1");
    ...
    public UserProperty getProp(String name) {
    	Query query = entityManager.createQuery("select p from UserProperty p where p.name = ? order by id desc limit 1");
