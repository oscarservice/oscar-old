...
public class LoginFilter implements Filter {
	...
	private static final String[] EXEMPT_URLS = { 
		...
		"/oscarEncounter/oscarConsultationRequest/ConsultationFormPrint.jsp",
		"/form/formname.do"
	};
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (!CRHelper.isCRFrameworkEnabled()) {
			...
			if (httpRequest.getSession().getAttribute("user") == null) {
				...
				{
					//for ICFHT demo export from scheduler
					if(requestURI.indexOf("ConsultationFormPrint.jsp")>0)
					{
						String fromScheduler = request.getParameter("fromScheduler");
						if(fromScheduler==null || !fromScheduler.equalsIgnoreCase("true"))
						{
							httpResponse.sendRedirect(contextPath + "/logout.jsp");
							return;
						}
					}
					else if(requestURI.indexOf("/form/formname.do")>0)
					{
						String fromScheduler = request.getParameter("fromScheduler");
						if(fromScheduler==null || !fromScheduler.equalsIgnoreCase("true"))
						{
