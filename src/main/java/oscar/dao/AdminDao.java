...
public class AdminDao extends OscarSuperDao {
	...
	private String [][] dbQueries = new String[][] {
	    ...
	    {"site_provider_search_providerno", "select p.provider_no, p.last_name, p.first_name from provider p INNER JOIN providersite s ON p.provider_no = s.provider_no WHERE p.status='1' and s.site_id IN (SELECT site_id from providersite where provider_no = ?) order by last_name"}
	};
