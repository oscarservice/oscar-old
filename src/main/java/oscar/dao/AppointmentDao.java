...
public class AppointmentDao extends OscarSuperDao {
	...
	private String [][] dbQueries = new String[][] {
            {"search_appt", "select count(appointment_no) AS n from appointment where appointment_date = ? and provider_no = ? and status not in ('C','D') and ((start_time>= ? and start_time<= ?) or (end_time>= ? and end_time<= ?) or (start_time<= ? and end_time>= ?) ) and program_id=? and location like ?" },
            {"search_appt_name", "select name from appointment where appointment_date = ? and provider_no = ? and status not in ('C','D') and ((start_time>= ? and start_time<= ?) or (end_time>= ? and end_time<= ?) or (start_time<= ? and end_time>= ?) ) and program_id=? and location like ?" },
