...
public class DmsInboxManageAction extends DispatchAction {
	...
	public ActionForward prepareForIndexPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			cData.populateCountsAndPatients(providerNo);
	...
	private Collection<LabResultData> filterLabDocsForSuperSite(Collection<LabResultData> labdocs, String userId)
	{
		if(labdocs==null || labdocs.size()==0)
			return labdocs;
		
		List<LabResultData> list = new ArrayList<LabResultData>(labdocs);
		List<LabResultData> resultList = filterLabDocsForSuperSite(list, userId);
		
		return resultList;
	}
	
	private List<LabResultData> filterLabDocsForSuperSite(List<LabResultData> labdocs, String userId)
	{
		if(labdocs==null || labdocs.size()==0)
			return labdocs;
		
		ArrayList<LabResultData> resultList = new ArrayList<LabResultData>();
		DemographicData demographicData = new DemographicData();
		
		SuperSiteUtil superSiteUtil = SuperSiteUtil.getInstance(userId);
		for (LabResultData labResultData : labdocs)
		{
			if(labResultData!=null)
			{
				String demographicId = "";
				
				if(labResultData.getLabPatientId()!=null && 
						labResultData.getLabPatientId().length()>0)
				{
					//in case lab type = doc.. then it will have patient id
					demographicId = labResultData.getLabPatientId();
				}
				else if(labResultData.getHealthNumber()!=null && labResultData.getHealthNumber().length()>0)
				{
					//if lab type is not doc.. seach patient id by hin
					String hin = labResultData.getHealthNumber();
					ArrayList<Demographic> demoList = demographicData.getDemographicWithHIN(hin);
					if(demoList!=null && demoList.size()>0)
					{
						//lab assigned to demographic.. check user access rights
						demographicId = demoList.get(0).getDemographicNo()+"";
					}
				}
				
				if(demographicId!=null && demographicId.length()>0)
				{
					//lab/doc assigned to demo.. check user access rights
					boolean flg = superSiteUtil.isUserAllowedToOpenPatientDtl(demographicId);
					if(flg)
					{
						//user has access to demographic.. display it
						resultList.add(labResultData);
					}
					else
					{
						//user doesnt have access to demographic.. filter next record
						continue;
					}
				}
				else
				{
					//doc/lab not assigned to any demographic.. display it
					resultList.add(labResultData);
				}
			}
		}
