...
public class MsgDisplayMessagesBean implements java.io.Serializable  {
  ...
  private SuperSiteUtil superSiteUtil;
 /*
 ...
 */
   ...
   */
   public void setProviderNo(String providerNo){
     ...
     superSiteUtil = SuperSiteUtil.getInstance(providerNo);
   }
  ...
  void getMessageIDs(){
     ...
     try{
        ...
        String sql = new String("select message, status, map.demographic_no from messagelisttbl t1 left outer join msgDemoMap map on map.messageID = t1.message where provider_no = '"+ providerNo+"' and status not like \'del\' and remoteLocation = '"+getCurrentLocationId()+"' order by message");
...
*/
        ...
        while (rs.next()) {
            ...
            if(dm.demographic_no!=null && dm.demographic_no.trim().length()>0)
            {
	            if(!superSiteUtil.isUserAllowedToOpenPatientDtl(dm.demographic_no))
	            	continue;
            }
