...
public class FrmCustomedPDFServlet extends HttpServlet {
    ...
    public void service(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException, java.io.IOException {		
		...
		oscar.oscarRx.pageUtil.RxSessionBean beanRX = (oscar.oscarRx.pageUtil.RxSessionBean) req.getSession().getAttribute("RxSessionBean");
		if(beanRX != null) {
			optometristEyeParam = beanRX.getOptometristEyeParam();
			req.setAttribute("optometristEyeParam", optometristEyeParam);
		}
	...
	protected ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, final ServletContext ctx) throws DocumentException {
		...
		try {
			...
			{
				bf = BaseFont.createFont(BaseFont.COURIER, BaseFont.CP1252, BaseFont.NOT_EMBEDDED);
				Paragraph p = new Paragraph(new Phrase("Eye "+(String)req.getAttribute("optometristEyeParam"), new Font(bf, 10)));
				p.setKeepTogether(true);
