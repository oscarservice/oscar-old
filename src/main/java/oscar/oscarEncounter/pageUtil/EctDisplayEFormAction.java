	...
	try
	{
		...
		if(!a) {
			...
			for (EFormData eFormData : eFormDatas)
			{
	            ...
	            url += "return false;";
	            item.setURL(url);
	            // Set the title name as required as the subject name 
	            if(!eFormData.getSubject().equals("")){
	            String strTitle = StringUtils.maxLenString(eFormData.getSubject(), MAX_LEN_TITLE, CROP_LEN_TITLE, ELLIPSES);
