...
public class EctConEditSpecialistsAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ServiceSpecialistsDao serviceSpecialistsDao=(ServiceSpecialistsDao)SpringUtils.getBean("serviceSpecialistsDao");
		...
		if (delete.equals(oscarR.getString("oscarEncounter.oscarConsultationRequest.config.EditSpecialists.btnDeleteSpecialist"))) {
			if (specialists.length > 0) {
				...
				{
					//check	consultationRequests specId existing? If no, then delete					
					int conRequestNumber = consultationRequestDao.getCountConRequestWithSpecId(Integer.parseInt(specialists[i]));
					if(conRequestNumber <= 0) {
						professionalSpecialistDao.remove(Integer.parseInt(specialists[i]));
						//Also remove from serviceSpecialists.
						List<ServiceSpecialists> ss = serviceSpecialistsDao.findBySpecId(Integer.parseInt(specialists[i]));
						for(ServiceSpecialists s : ss) {
							serviceSpecialistsDao.remove(s.getId());
						}						
					} else {
						request.setAttribute("DELETE_SPECIALIST", "DELETE FAILED AS IT IS USED IN CONSULATION REQUEST.");
					}
				}
			}
