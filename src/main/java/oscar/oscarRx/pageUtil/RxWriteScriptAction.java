...
public final class RxWriteScriptAction extends DispatchAction {
	...
	private static final FavoriteDao favoriteDao = (FavoriteDao)SpringUtils.getBean("favoriteDao");
	...
	public ActionForward updateSaveAllDrugs(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, Exception {
		...
		String optometristEyeParam = null;
				...
				if (stashIndex == -1) {
					...
					while (em.hasMoreElements()) {
						...
						if (elem.startsWith("drugName_" + num)) {
							...
							rx.setDrugForm(val);
						} else if (elem.equals("optometristEyeParam")) {
							bean.setOptometristEyeParam(val);
						}	
					}
	...
	public ActionForward addFavorite(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		oscar.oscarRx.pageUtil.RxSessionBean bean = (oscar.oscarRx.pageUtil.RxSessionBean) request.getSession().getAttribute("RxSessionBean");
		List<String> paramList = new ArrayList();
		Enumeration em = request.getParameterNames();
		List<String> randNum = new ArrayList<String>(); // list of drugs, line by line
		while (em.hasMoreElements()) {
			String ele = em.nextElement().toString();
			paramList.add(ele);
			if (ele.startsWith("drugName_")) {
		...
		List<Integer> existingIndex = new ArrayList<Integer>();
		List<RxPrescriptionData.Prescription> pseudoPrescriptions = new ArrayList<RxPrescriptionData.Prescription>();
		for (String num : randNum) {
			int stashIndex = bean.getIndexFromRx(Integer.parseInt(num));
			try {
				if (stashIndex == -1) {
		...
		saveFavorite(providerNo, pseudoPrescriptions);
		
        List<Favorite> favorites = favoriteDao.listFavorites(providerNo);
		response.setContentType("text/x-json");
        if(favorites.size() > 0) {
        	JSONObject jsonObject = new JSONObject();
        	for(Favorite f : favorites) {
            	jsonObject = jsonObject.accumulate("favorites", f);        		
        	}
	...
	public void saveFavorite(String providerNo, List<RxPrescriptionData.Prescription> pseudoPrescriptions) {
		for (int i = 0; i < pseudoPrescriptions.size(); i++) {
			RxPrescriptionData.Prescription prescr = pseudoPrescriptions.get(i);
			Favorite favorite = new Favorite();
			favorite.setAtc(prescr.getAtcCode());
			favorite.setBn(prescr.getBrandName());
			favorite.setCustomInstructions(false);
			favorite.setCustomName(prescr.getCustomName()!=null && prescr.getCustomName().length()>60 ? prescr.getCustomName().substring(0,60) : null);
			favorite.setDosage(prescr.getDosage());
			favorite.setDrugForm(prescr.getDrugForm()!=null && prescr.getDrugForm().length()>50 ? prescr.getDrugForm().substring(0,50) : prescr.getDrugForm());
			favorite.setDuration(prescr.getDuration());
			favorite.setDurationUnit(prescr.getDurationUnit());
			favorite.setFrequencyCode(prescr.getFrequencyCode());
			favorite.setGcnSeqno(prescr.getGCN_SEQNO());
			favorite.setGn(prescr.getGenericName());
			favorite.setMethod(prescr.getMethod());
			favorite.setName(prescr.getBrandName());
			favorite.setNosubs(prescr.getNosubs());
			favorite.setPrn(prescr.getPrn());
			favorite.setProviderNo(providerNo);
			favorite.setQuantity(prescr.getQuantity());
			favorite.setRegionalIdentifier(prescr.getRegionalIdentifier());
			favorite.setRepeat(prescr.getRepeat());
			favorite.setRoute(prescr.getRoute());
			favorite.setSpecial(prescr.getSpecial());
			favorite.setTakeMax(prescr.getTakeMax());
			favorite.setTakeMin(prescr.getTakeMin());
			favorite.setUnit(prescr.getUnit());
			favorite.setUnitName(prescr.getUnitName());
