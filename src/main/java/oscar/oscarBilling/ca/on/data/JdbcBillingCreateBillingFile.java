...
public class JdbcBillingCreateBillingFile {
	...
	private String buildHeader1() {
		...
		_logger.info("Build billing header1(id = " + ch1Obj.getId() + ")");
	...
	private String buildHTMLContentRecord(int invCount,boolean simulation) {
		...
		String bgColorErrorStr = "";
		if(errorPartMsg!=null && errorPartMsg.length()>0)
		{
			bgColorErrorStr = " bgcolor='yellow' ";
			styleClass = "";
		}
		
		if (invCount == 0) {			
			Demographic demo = demographicDao.getDemographic(ch1Obj.getDemographic_no());
			ret += "\n<tr "+bgColorErrorStr+" "+(summaryView ? "style='display:none;' class='record"+providerNo+"'": "")+">"; 
			if (simulation) {
			...
			ret = "\n<tr "+bgColorErrorStr+" "+(summaryView ? "style='display:none;' class='record"+providerNo+"'": "")+">"+ "<td class='" + styleClass + "'>&nbsp;</td>" + "<td class='" + styleClass + "'>&nbsp;</td> <td class='" + styleClass + "'>&nbsp;</td><td class='" + styleClass + "'>&nbsp;</td><td class='" + styleClass + "'>&nbsp;</td><td class='" + styleClass + "'>&nbsp;</td>"
	...
	private String buildSiteHTMLContentRecord(int invCount) {
		...
		if (invCount == 0) {
			ret = "\n<tr "+bgColorErrorStr+"><td class='"+cellClass+"'><a href=# onclick=\"popupPage(720,640,'billingONCorrection.jsp?billing_no="
					+ ch1Obj.getId()
					+ "');return false;\">"
					+ ch1Obj.getId()
					+ "</a></td>"
					+ "<td class='"+cellClass+"'><a href=# onclick=\"popupPage(720,740,'../../../demographic/demographiccontrol.jsp?demographic_no="
					+ ch1Obj.getDemographic_no()
					+ "&displaymode=edit&dboperation=search_detail');return false;\">"
					+ ch1Obj.getDemographic_name()
					+ "</a></td><td class='"+cellClass+"'>"
					+ ch1Obj.getHin() + ch1Obj.getVer()
					+ "</td><td class='"+cellClass+"'>"
					+ ch1Obj.getBilling_date()
					+ "</td><td class='"+cellClass+"'>"
					+ itemObj.getService_code()
					+ "</td><td align='right' class='"+cellClass+"'>"
					+ itemObj.getFee()
					+ "</td><td align='right' class='"+cellClass+"'>"
					+ itemObj.getDx()
					+ "</td><td class='"+cellClass+"'> &nbsp; &nbsp;" + referral + hcFlag + m_Flag + " </td>" 
					+ "<td bgcolor='" + clinicBgColor + "'> " + clinicShortName.get(ch1Obj.getClinic()) + "</td></tr>";
		} else {
			ret = "\n<tr "+bgColorErrorStr+"><td class='"+cellClass+"'>&nbsp;</td> <td class='"+cellClass+"'>&nbsp;</td>"
					+ "<td class='"+cellClass+"'>&nbsp;</td> <td class='"+cellClass+"'>&nbsp;</td>" + "<td class='"+cellClass+"'>"
					+ itemObj.getService_code() + "</td><td align='right' class='"+cellClass+"'>" + itemObj.getFee()
					+ "</td><td align='right' class='"+cellClass+"'>" + itemObj.getDx()
					+ "</td><td class='"+cellClass+"'>&nbsp;</td>" 
					+ "<td bgcolor='" + clinicBgColor + "'> " + clinicShortName.get(ch1Obj.getClinic()) + "</td></tr>";
		}
		return ret;
	}
	private String buildHTMLContentTrailer(boolean simulation) {
		...
		if (error > 0) { 
			if (errorCount > 3) {
				...
				errorMsgHtml  = "<tr bgcolor='yellow'><td colspan='12'><font color='black'>" 
				...
				errorMsgHtml = "<tr bgcolor='yellow'><td colspan='12'><font color='black'>" + totalError + "</font></td></tr>";
			}
		}
		if (error == 0) {
	...
	public void createBillingFileStr(String bid, String status, boolean simulation, String mohOffice, boolean summaryView, boolean useProviderMOH) {
		...
		try {
			...
			while (rs.next()) {
				...
				if(simulation)
					checkOhipFileLinesLength(bNo, chkFileLineLen_LineNo);
			}
	...
	//added by rohit for checking ohip file line length while simulating billing
	private static final int OHIP_FILE_LINE_LEN_BYTES = Integer.parseInt(OscarProperties.getInstance().getProperty("OHIP_FILE_LINE_LEN_BYTES", "81"));
	private int chkFileLineLen_LineNo = 0;
	//chkFileLineLen_LineNo = last processed line no.
	public void checkOhipFileLinesLength(String invoiceNo, int lastProcessedLineNo)
	{
		try
		{
			String fileContent = this.getValue();	
			_logger.info("fileContent = "+fileContent);
			if(fileContent==null)
				return;
			
			File file = File.createTempFile("SIMULATE_OHIP_TEMP_FILE", "");
			_logger.info("file.getPath() = "+file.getPath());
			...
			{
				if(lineNo==1 || lineNo<=lastProcessedLineNo)
				{
					lineNo++;
					continue;
				}
				...
				if(str.trim().length()>0)
				{
					...
					_logger.info(str);
					_logger.info(lineNo +" byte len - "+bytesLen);
					_logger.info("-----------------------------------------");
					
					if(bytesLen!=OHIP_FILE_LINE_LEN_BYTES)
					{
						_logger.info("Line No. - "+lineNo+" - bytes length is "+bytesLen+", Required length is - "+OHIP_FILE_LINE_LEN_BYTES+", Invoice No.: "+invoiceNo);
						this.errorMsg += "Line No. - "+lineNo+" - bytes length is "+bytesLen+", Required length is - "+OHIP_FILE_LINE_LEN_BYTES+", Invoice No.: "+invoiceNo+"<br>";
					}
				}
				this.chkFileLineLen_LineNo = lineNo++;
			}
		}
		catch (Exception e)
		{
			_logger.error("Error while checking ohip file lines length", e);			
		}
	}
