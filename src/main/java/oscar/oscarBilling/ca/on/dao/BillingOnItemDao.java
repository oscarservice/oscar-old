...
public class BillingOnItemDao extends HibernateDaoSupport {
	...
	// String queryStr =
	// "FROM BillingONPayment b WHERE b.ch1_id = "+BillingNo+" ORDER BY b.id desc";
	//
	// @SuppressWarnings("unchecked")
	...
	public void updateItemPayment(BillingOnItem item, String paid,
			String discount) {
	...
	public void updateItemPaymentNew(BillingOnPaymentItem item, String paid,
			String discount) {
		item.setPaid(new BigDecimal(paid));
		item.setDiscount(new BigDecimal(discount));
		// item.setBilling_on_payment_id(paymentId);
	...
	/*public void addUpdateOneBillItemTrans(BillingClaimHeader1Data billHeader,
			BillingOnItem item, String updateProviderNo, String pay_ref,
			String discount, BillingOnPaymentItem item1) {
	...
	public void addUpdateOneBillItemTransForRefund(
			BillingClaimHeader1Data billHeader, BillingOnItem item,
			String updateProviderNo, String pay_ref, String discount,
			String refund, BillingOnPaymentItem item1) {
		StringBuffer sqlBuf = new StringBuffer();
		sqlBuf.append("insert into billing_on_transaction values");
		sqlBuf.append("(\\N,");
		sqlBuf.append(item.getCh1_id() + ","); // cheader1_id
