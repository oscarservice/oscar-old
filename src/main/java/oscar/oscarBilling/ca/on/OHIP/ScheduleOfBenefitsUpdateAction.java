	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)  {
		...
		if (forceUpdate) { 
			...
			for (int i = 0; i < codes.size(); i++) {
				...
				int count_code;
				count_code = bc.searchNumBillingCode((String)code.get("feeCode"));
				if(count_code >= 1){
					bc.editBillingCodeDesc((String)code.get("description"), code.get("newprice").toString(), (String)code.get("feeCode"));
				}else{
			...
			if ( changes != null ){
				...
				for ( int i = 0 ; i < changes.length; i++){
					...
					if (change != null && change.length == 5){
						...
						int count_code;
						count_code = bc.searchNumBillingCode(change[0]);
						if(count_code >= 1){
							bc.editBillingCodeDesc(change[4], change[1], change[0]);
						}else{
