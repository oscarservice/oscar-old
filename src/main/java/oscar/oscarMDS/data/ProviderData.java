...
public class ProviderData {
    ...
    public static ArrayList<ArrayList<String>> getProviderList () {
        try {
            ...
            String sql = "select provider_no, first_name, last_name from provider where status='1' and provider_type='doctor' order by last_name , first_name";
    ...
    public static ArrayList<ArrayList<String>> getProviderListWithLabNo () {
        try {
            ...
            String sql = "select provider_no, first_name, last_name from provider where status='1' and provider_type='doctor'  and ohip_no != '' order by last_name , first_name";
            ResultSet rs = DBHandler.GetSQL(sql);
            while ( rs.next() ) {
                ArrayList<String> provider = new ArrayList<String>();
                provider.add(oscar.Misc.getString(rs, "provider_no"));
