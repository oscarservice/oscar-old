...
<ul id="<%=request.getAttribute("navbarName")%>list">
	<%
            ...
            //added by rohit (rohitprajapati54@gmail.com).. whether to display all items in the section as expanded or not
            if(request.getParameter("displayAllItems")!=null && request.getParameter("displayAllItems").equalsIgnoreCase("true"))
            {
            	if(request.getParameter("displayAllProp")!=null && request.getParameter("displayAllProp").trim().length()>0)
            	{
            		//if proeprty is there.. then check whether its true/false
            		String displayAllPropVal = OscarProperties.getInstance().getProperty(request.getParameter("displayAllProp").trim());
            		if(displayAllPropVal!=null && displayAllPropVal.equalsIgnoreCase("true"))
            		{
            			strToDisplay = numItems+"";
            		}
            	}
            	else
            		strToDisplay = numItems+"";
