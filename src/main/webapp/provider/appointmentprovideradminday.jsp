...
String selectedSite = (String)session.getAttribute("site_selected");
...
List providersData = privateProvidersUtil.getPrivateProvidersData(
			bMultisites, isTeamAccessPrivacy, 
			isSiteAccessPrivacy, 
			curUser_no, selectedSite, 
			isNoGroupsForDoctors);
			
List<Site> sites = (List<Site>) providersData.get(0);
List<Site> curUserSites = (List<Site>) providersData.get(1);
List<String> providerNos = new ArrayList<String>();
List<Provider> providers = (List<Provider>) providersData.get(2);
List<String> siteGroups = (List<String>) providersData.get(3);
...
if(providerNos.size()>0) session.setAttribute("providerNoSearchList",providerNos);
...
.reason {
    ...
    font: 14px Times New Roman; font-style: italic;
}
</style>
...
<caisi:isModuleLoad moduleName="TORONTO_RFQ" reverse="true">
  ...
  <li>
     <a href=<%=OscarProperties.getInstance().getProperty("FAX_URL", "http://192.168.1.151/avantfax")%> target="_blank">FAX</a>
  </li>
  <li id="admin">
...
<oscar:oscarPropertiesCheck property="ENABLE_APPT_DOC_COLOR" value="yes">
	  ...
	  <%}}%>
      <oscar:oscarPropertiesCheck property="SHOW_APPT_REASON" value="yes">| <i><%=reason%></i></oscar:oscarPropertiesCheck>
	  </security:oscarSec>
	...
	<oscar:oscarPropertiesCheck property="appt_pregnancy" value="true" defaultVal="false">
      ...
      <oscar:oscarPropertiesCheck property="SHOW_APPT_REASON_TOOLTIP" value="yes" defaultVal="true"><span class="reason"><%=UtilMisc.htmlEscape(reason)%></span></oscar:oscarPropertiesCheck>
      <% } %>
<% } %>
        		</font></td>
        <%
