  ...
  while (bFistEntry?it.hasNext():true) { 
    ...
    if(isTeamOnly || !providerview.startsWith("_grp_",0) || myGrpBean.containsKey(String.valueOf(date.get("provider_no"))) ) {
		...
		if(app_name.length() == 0){
			app_name = app_name.append(providerNameBean.getShortDef(String.valueOf(date.get("provider_no")),"",NameMaxLen ));
			print_name = true;
		}
		else{
			String name[] = app_name.toString().split("-");
			
			for(int count = 0;count<name.length;count++){
				if(name[count] == providerNameBean.getShortDef(String.valueOf(date.get("provider_no")),"",NameMaxLen )){
					print_name = false;
				}
				else{
					app_name.append(providerNameBean.getShortDef(String.valueOf(date.get("provider_no")),"",NameMaxLen ) + "-");
					print_name = true;
				}				
			}
		}
