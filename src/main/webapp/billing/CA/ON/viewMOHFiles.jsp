...
<%
    ...
    if(!(folderPath.endsWith("\\") || folderPath.endsWith("/"))) folderPath +="/";
%>
...
<%@ page import="java.util.*,oscar.*,java.io.*,java.net.*,oscar.util.*,org.oscarehr.util.*,org.apache.commons.io.FileUtils,java.text.SimpleDateFormat,org.oscarehr.billing.CA.ON.util.EDTFolder;" errorPage="errorpage.jsp"%>
...
function viewMOHFile (filename) {
	...
	if (filename.substring(filename.length-4).toLowerCase() == ".zip") {
		var r=alert("          Trying unzip "+filename+",\n unzipped file(s) should be at the top of the list");
		if(location.href.toString().indexOf("?")!=-1) location.href=location.href+"&unzipfile="+filename;
		else location.href=location.href+"?unzipfile="+filename;
		return;
		...
		form.action ="/<%= OscarProperties.getInstance().getProperty("project_home") %>/billing/CA/ON/displayMOHXML.do";
	} else {
...
<table cellspacing="1" cellpadding="2" width="90%" border="0">
   ...
   String zname = (String)request.getParameter("unzipfile");
   String unzipMSG = "";
   try {
     if ( zname != null && !zname.equals("") ) {
         Boolean unzipDone = zip.unzipXMLbyStream(folderPath,zname);
    ...
    for(int i=0; i<contents.length; i++) {
      ...
      if (folder == EDTFolder.INBOX || folder == EDTFolder.ARCHIVE) {
          out.println("<tr bgcolor='"+ (bodd?weakcolor:"white") +"'>"+(folder == EDTFolder.INBOX ? archiveElement : "")+"<td><a HREF='#' onclick='viewMOHFile(\""+URLEncoder.encode(contents[i].getName())+"\")'>"+contents[i].getName()+(zname==contents[i].getName() ? unzipMSG : "")+"</a></td>") ;
