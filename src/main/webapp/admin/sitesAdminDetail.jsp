...
<%@page import="com.quatro.dao.security.SecroleDao"%>
...
<%
    ...
    String siteId = (String) request.getParameter("siteId");
   	...
   	String logoError = "";	
	logoError = (String)request.getParameter("logoErrors");
...
<script type="text/javascript" src="<%= request.getContextPath() %>/js/jquery-ui-1.7.3.custom.min.js"></script>
<script>
     jQuery.noConflict();
</script>
...
<%
	...
	{
		siteRoleList = superSiteUtil.getAccessRolesAssociatedWithSite(Integer.parseInt(currentSiteId));
		siteAdmitDischargeRoleList = superSiteUtil.getAdmitDischargeRolesAssociatedWithSite(Integer.parseInt(currentSiteId));
	}
			...
			<div id="div_access_roles" style="float: left;">
				...
				<ul>
					...
					<%for(int i=0;i<siteRoleList.size();i++){ %>
						<li class="li_role" ><%=siteRoleList.get(i).getRoleName() %>
						<a href="#" title="Delete Role" roleType="access" roleName="<%=siteRoleList.get(i).getRoleName() %>" roleId="<%=siteRoleList.get(i).getId() %>" onclick="onClickDeleteRole(this);"></a>
						</li>
					<%} %>
				<%} %>
			...
			<div id="div_access_roles" style="float: left;">
				...
				<%if(siteAdmitDischargeRoleList!=null && siteAdmitDischargeRoleList.size()>0){ %>
					<%for(int i=0;i<siteAdmitDischargeRoleList.size();i++){ %>
						<li class="li_role"><%=siteAdmitDischargeRoleList.get(i).getRoleName() %>
						<a href="#"  title="Delete Role" roleType="admit_discharge" roleName="<%=siteAdmitDischargeRoleList.get(i).getRoleName() %>" roleId="<%=siteAdmitDischargeRoleList.get(i).getId() %>" onclick="onClickDeleteRole(this);"></a>
						</li>
					<%} %>
				<%} %>
					<li class="li_add_role_btn">
...
function getAccessRoleOptions()
{
	...
	if(allRoleList!=null && allRoleList.size()>0)
	{
		for(int i=0;i<allRoleList.size();i++)
		{
			...
			{
				str = str+"<option value='"+secrole.getId()+"'>"+secrole.getRoleName()+"</option>";
			}
		}
	}
	%>
...
function getAdmitDischargeRoleOptions()
{
	...
	if(allRoleList!=null && allRoleList.size()>0)
	{
		for(int i=0;i<allRoleList.size();i++)
		{
			...
			{
				str_ = str_+"<option value='"+secrole.getId()+"'>"+secrole.getRoleName()+"</option>";
			}
		}
	}
	%>
