...
<%
String[] statusArr = request.getParameterValues("search_status");
String searchStatusRequestParamStr = "";
if(statusArr!=null && statusArr.length>0)
{	
	for(int i=0;i<statusArr.length;i++)
	{
		searchStatusRequestParamStr = searchStatusRequestParamStr+"&search_status="+statusArr[i];
	}
}
%>
...
<table width="100%" cellspacing="2" cellpadding="2" border="0"
	...
	<tr bgcolor="<%=deepcolor%>">
		<TH align="center" width="10%"><b><a
			href="admincontrol.jsp?keyword=<%=request.getParameter("keyword")%>&search_mode=<%=request.getParameter("search_mode")%>&displaymode=<%=request.getParameter("displaymode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=<%="provider_no"%>&limit1=0&limit2=10<%=searchStatusRequestParamStr%>"><bean:message
			key="admin.providersearchresults.ID" /></a></b></TH>
		<TH align="center" width="19%"><b><a
			href="admincontrol.jsp?keyword=<%=request.getParameter("keyword")%>&search_mode=<%=request.getParameter("search_mode")%>&displaymode=<%=request.getParameter("displaymode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=<%="first_name"%>&limit1=0&limit2=10<%=searchStatusRequestParamStr%>"><bean:message
			key="admin.provider.formFirstName" /></a> </b></TH>
		<TH align="center" width="19%"><b><a
			href="admincontrol.jsp?keyword=<%=request.getParameter("keyword")%>&search_mode=<%=request.getParameter("search_mode")%>&displaymode=<%=request.getParameter("displaymode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=<%="last_name"%>&limit1=0&limit2=10<%=searchStatusRequestParamStr%>"><bean:message
...
<%
  ...
  if(nLastPage>=0) {
%> <a
	href="admincontrol.jsp?keyword=<%=request.getParameter("keyword")%>&search_mode=<%=request.getParameter("search_mode")%>&displaymode=<%=request.getParameter("displaymode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=<%=request.getParameter("orderby")%>&limit1=<%=nLastPage%>&limit2=<%=strLimit2%><%=searchStatusRequestParamStr%>"><bean:message
	key="admin.providersearchresults.btnLastPage" /></a> | <%
  }
  if(nItems==Integer.parseInt(strLimit2)) {
%> <a
	href="admincontrol.jsp?keyword=<%=request.getParameter("keyword")%>&search_mode=<%=request.getParameter("search_mode")%>&displaymode=<%=request.getParameter("displaymode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=<%=request.getParameter("orderby")%>&limit1=<%=nNextPage%>&limit2=<%=strLimit2%><%=searchStatusRequestParamStr%>"><bean:message
