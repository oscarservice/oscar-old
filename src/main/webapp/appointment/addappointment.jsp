...
<% if(apptObj!=null) { 
	String apptReason = apptObj.getReason().replaceAll("[\r\n]","");
	String apptNote = apptObj.getNotes().replaceAll("[\r\n]", "");
%>
function pasteAppt(multipleSameDayGroupAppt) {
        ...
        document.forms[0].reason.value = "<%=apptReason%>";
        document.forms[0].notes.value = "<%=apptNote%>";
...
<%
  ...
  String siteSelected = (String)session.getAttribute("site_selected");
  if(!org.apache.commons.lang.StringUtils.isBlank(siteSelected) && !siteSelected.equalsIgnoreCase("null") )
 	 param[9] = (String)session.getAttribute("site_selected");
  else
	 param[9] = "%";
        ...
        <li class="row weak">
            ...
            <%
				    ...
				    if(bFirstDisp && bMoreAddr) {
			            	...
			            	if(StringUtils.isBlank(tempLoc)) {

			            		tempLoc = (String)session.getAttribute("site_selected");
