                        ...
                        <tr><!--put this left-->
							...
							<%if(securityManager.hasWriteAccess("_rx",roleName$,true)) {%>
                                <html:form action="/oscarRx/searchDrug"  onsubmit="return checkEnterSendRx();" style="display: inline; margin-bottom:0;" styleId="drugForm">
                                    ...
                                    <%--commented by rohit : as per bug #95 --%>
								<%-- <% if(OscarProperties.getInstance().getProperty("cme_js")!=null && !OscarProperties.getInstance().getProperty("cme_js").equals("")) { %>
                                    <span style="padding-left:5px"/>Eye: <span style="padding-left:5px"/> 
	                            <input type="radio" name="optometristEyeParam" value="OD" /> OD
	                            <input type="radio" name="optometristEyeParam" value="OS" /> OS
	                            <input type="radio" name="optometristEyeParam" value="OV" /> OU
	                            <br/>
	                            <%} %> --%>
                                    <table border="0">
                                        <tr valign="top">
                                            <td style="width:320px;"><bean:message key="SearchDrug.drugSearchTextBox"  />
                                                ...
                                                <span style="padding-left:15px">
                                                    <input type="button" class="ControlPushButton" style="width:106px" onclick="addFavorite()" value="<bean:message key="SearchDrug.addFavorite"/>"/>
                                                </span>    
                                            </td>
                                            <td>
                                                <span style="padding-left:15px">
...
function addFavorite() {
        var data=Form.serialize($('drugForm'));
        var url= "<c:out value="${ctx}"/>" + "/oscarRx/WriteScript.do?parameterValue=addFavorite";
        new Ajax.Request(url,
        {method: 'post',postBody:data,asynchronous:false,
            onFailure:function(transport){
                alert('Failed to save this drug to favorites');
            },
            onSuccess:function(transport){
                var data = transport.responseText.evalJSON();
                var result = data.result;
                var favoritesHTML = "";
                if(result == "empty") alert('Not saved');
                else {
                    var favorites = data.favorites;
                    for(i=0; i<favorites.length; i++) {
                        var fid = favorites[i].id;              
                        var fullName = favorites[i].name;
                        var shortName = fullName; 
                        if(shortName.length>20) shortName = shortName.substring(0,17)+'...';
                        favoritesHTML +="<p class=\"PropSheetMenuItemLevel1\"><input type=\"checkbox\" id=\"checkfav_"+fid+"\" checked=\"false\"><a href=\"javascript:void(0);\" onclick=\"useFav2('"+fid+"');\" title=\""+fullName+"\">"+shortName+"</a></p>";
                    }
                }
...
function prescribeFavorites() {
	$$('input[type="checkbox"][id^="checkfav"]').each(function(item,index){
	   var strId = ''+item.id;
	   strId = strId.substring(strId.indexOf('_')+1, strId.length);
	   if(item.checked) useFav2(strId);
        }); 
}
