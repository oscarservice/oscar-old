...
////////////////////////////////////////////////////////////////////////
		...
		if(docassigned != null && docassigned.equals("."+ResourceBundle.getBundle("oscarResources", request.getLocale()).getString("global.default"))) {
		    WebApplicationContext ctx = WebApplicationContextUtils.getRequiredWebApplicationContext(getServletContext());
		    ProviderDataDao providerDataDao = (ProviderDataDao) ctx.getBean("providerDataDao");
		    List<ProviderData> providers = providerDataDao.searchDoctors();			
		    for(ProviderData prv : providers) {
				Tickler tickler = new Tickler();
    			tickler.setDemographic_no(module_id);
    			tickler.setStatus('A');
    			tickler.setUpdate_date(new java.util.Date());
    			tickler.setPriority(docpriority);
    			tickler.setTask_assigned_to(prv.getId());
		...
		} if(docassigned != null && docassigned.startsWith("_grp_")) {
		    String groupNo = docassigned.substring(5);
		    WebApplicationContext ctx = WebApplicationContextUtils.getRequiredWebApplicationContext(getServletContext());
		    MyGroupDao myGroupDao = (MyGroupDao) ctx.getBean("myGroupDao");
		    List<String> providers = myGroupDao.getGroupDoctors(groupNo);
		    for(String prv : providers) {
    			...
    			tickler.setUpdate_date(new java.util.Date());
    			tickler.setPriority(docpriority);
    			tickler.setTask_assigned_to(prv);
    			tickler.setCreator(doccreator);
    			tickler.setMessage(docfilename);
    			tickler.setService_date(UtilDateUtilities.StringToDate(docdate));
		    	ticklers.add(tickler);
