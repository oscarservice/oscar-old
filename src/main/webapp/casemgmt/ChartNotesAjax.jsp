...
<c:if test="${not empty notesToDisplay}">
	<%
		...
		UserProperty uProp = caseManagementManager.getUserProperty(provNo, UserProperty.STALE_NOTEDATE);
		...
		*/
		...
		for (idx = 0; idx < noteSize; ++idx)
		{
			...
			boolean hideNoteFooter = OscarProperties.getInstance().isPropertyActive("encounter.hide_note_footer");
			String noteFooterDisplay="block";
			if(hideNoteFooter) {
				noteFooterDisplay = "none";
			}
				...
				else //else display contents of note for viewing
				{
					...
					else
					{
						...
						if (!note.isDocument() && !note.isCpp() && !note.isEformData() && !note.isEncounterForm() && !note.isInvoice())
						{
							...
							if (OscarProperties.getInstance().getBooleanProperty("note_program_ui_enabled", "true")) {
							%>
						 		<div style="display:<%=noteFooterDisplay %>" class ="_program" noteId="<%=globalNoteId %>" programName="<%=note.getProgramName() %>" roleName="<%=note.getRoleName() %>">
						 			<span class="program"><%=note.getProgramName() %> (<%=note.getRoleName() %>)</span>
						 		</div>
							<%
							}
						%>						
							<div id="sig<%=globalNoteId%>" style="display:<%=noteFooterDisplay %>" class="sig" style="clear:both;<%=bgColour%>">
...
<script type="text/javascript">	
	...
	jQuery(document).ready(function() {
    	...
    	UserProperty slProp = caseManagementManager.getUserProperty(provNo, UserProperty.STALE_FORMAT);
   ...
   //AutoCompleter for Issues
    ...
    <%int MaxLen = 20;
			...
			{
				String encounterTmpFullName = bean.templateNames.get(j);
     ...
     autoCompleted["<%=encounterTmp%>"] = "ajaxInsertTemplate('<%=encounterTmpFullName%>')";
     autoCompList.push("<%=encounterTmp%>");
     itemColours["<%=encounterTmp%>"] = "99CCCC";
   <%}%>
   new Autocompleter.Local('enTemplate', 'enTemplate_list', autoCompList, { colours: itemColours, afterUpdateElement: menuAction , choices : 30}  );
