...
if(org.oscarehr.common.IsPropertiesOn.isMultisitesEnable()){
	...
	if(request.getParameter("demographicNo")!=null)
		demographicId = request.getParameter("demographicNo").trim();
	
	SuperSiteUtil superSiteUtil = (SuperSiteUtil) SpringUtils.getBean("superSiteUtil");
	if(!superSiteUtil.isUserAllowedToOpenPatientDtl(userId, demographicId))
	{
		%>
		<h2><font color="red"><bean:message key="supersite.access.error"/></font></h2>
		<%	
		return;
	}
}
...
<%-- <%
SuperSiteUtil superSiteUtil = (SuperSiteUtil) SpringUtils.getBean("superSiteUtil");
if(!superSiteUtil.isUserAllowedToOpenPatientDtl(request, "demographicNo"))
{
	%>
	<jsp:forward page="/common/superSiteAccessError.jsp"></jsp:forward>
	<%
}
%> --%>
